const config = ui_mcm
  ? ({
      ...
      distance_to_campfire: ui_mcm.get(
        "pcprs_healing_campfires/distance_to_campfire",
      ),
    } satisfies McmConfig)
  : { ...pcprs_healing_campfires_mcm.defaultConfig }

const base_regen = 0.0003
...
function suitable_campfire_nearby(this: void, distance: number): boolean {
  ...
  for (const [, binder] of Object.entries(bind_campfire.campfires_all)) {
    ...
    if (
      !config.must_be_lit ||
      (config.must_be_lit && binder.campfire.is_on())
    ) {
...
function actor_on_update(this: void) {
  ...
  if (suitable_campfire_nearby(config.distance_to_campfire)) {
    actor.change_health(base_regen * config.hp_restore_mlt)
  }
}
...
function on_option_change(this: void) {
  if (ui_mcm) {
    ...
    config.distance_to_campfire = ui_mcm.get(
      "pcprs_healing_campfires/distance_to_campfire",
    )
  }
}
